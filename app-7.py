"""
app.py — plain Streamlit UI for the multi-agent research pipeline.
Imports Orchestrator + the compiled graph from pipeline.py. No custom
CSS/theming — default Streamlit look, as requested.
"""
...
st.caption("Planner → Retriever → Reader → Analyzer → Writer → Critic → Publisher, with a revision loop.")
...
with st.sidebar:
    st.header("Settings")
    query = st.text_area("Research query", placeholder="e.g. Impact of artificial intelligence on education", height=100)
...
if output is None:
    ...
    if not output.get("ok"):
        ...
        if output.get("events"):
            with st.expander("Event log up to the point of failure"):
    ...
    tab_report, tab_citations, tab_critic, tab_log = st.tabs(
        ["📄 Report", "🔗 Citations & Sources", "🕵️ Critic", "🛠️ Execution Log"]
    )
